...
public class OnSellContentAdapter extends RecyclerView.Adapter<OnSellContentAdapter.InnerHolder> {
    ...
    public void setData(OnSellContent content) {
        List<OnSellContent.DataBean.TbkDgOptimusMaterialResponseBean.ResultListBean.MapDataBean> contentData = content.getData().getTbk_dg_optimus_material_response().getResult_list().getMap_data();
        if (contentData.size() > 0) {
            mData.clear();
            ;
            mData.addAll(contentData);
